...
brpop key timeout = sendRequest (["BRPOP"] ++ map encode (toList key) ++ [encode timeout] )
...
sinter
    ...
    => NonEmpty ByteString -- ^ key
    -> m (f [ByteString])
sinter key = sendRequest (["SINTER"] ++ map encode (toList key) )
...
pfadd
    ...
    -> m (f Integer)
pfadd key value = sendRequest (["PFADD"] ++ [encode key] ++ map encode (toList value) )
...
lpush
    ...
    -> NonEmpty ByteString -- ^ value
    -> m (f Integer)
lpush key value = sendRequest (["LPUSH"] ++ [encode key] ++ map encode (toList value) )
...
pfcount
    ...
    => NonEmpty ByteString -- ^ key
    -> m (f Integer)
pfcount key = sendRequest (["PFCOUNT"] ++ map encode (toList key) )
...
hdel
    ...
    -> m (f Integer)
hdel key field = sendRequest (["HDEL"] ++ [encode key] ++ map encode (toList field) )
...
sunion
    ...
    => NonEmpty ByteString -- ^ key
    -> m (f [ByteString])
sunion key = sendRequest (["SUNION"] ++ map encode (toList key) )
...
sinterstore
    ...
    -> NonEmpty ByteString -- ^ key
    -> m (f Integer)
sinterstore destination key = sendRequest (["SINTERSTORE"] ++ [encode destination] ++ map encode (toList key) )
...
mget
    ...
    => NonEmpty ByteString -- ^ key
    -> m (f [Maybe ByteString])
mget key = sendRequest (["MGET"] ++ map encode (toList key) )
...
pfmerge
    :: (RedisCtx m f)
    => ByteString -- ^ destkey
    -> NonEmpty ByteString -- ^ sourcekey
    -> m (f ByteString)
pfmerge destkey sourcekey = sendRequest (["PFMERGE"] ++ [encode destkey] ++ map encode (toList sourcekey) )
...
sdiff
    ...
    => NonEmpty ByteString -- ^ key
    -> m (f [ByteString])
sdiff key = sendRequest (["SDIFF"] ++ map encode (toList key) )
...
sdiffstore
    ...
    -> NonEmpty ByteString -- ^ key
    -> m (f Integer)
sdiffstore destination key = sendRequest (["SDIFFSTORE"] ++ [encode destination] ++ map encode (toList key) )
...
rpush
    ...
    -> NonEmpty ByteString -- ^ value
    -> m (f Integer)
rpush key value = sendRequest (["RPUSH"] ++ [encode key] ++ map encode (toList value) )
...
mset
    ...
    -> m (f Status)
mset keyValue = sendRequest (["MSET"] ++ concatMap (\(x,y) -> [encode x,encode y]) (toList keyValue) )
...
scriptExists
    ...
    -> m (f [Bool])
scriptExists script = sendRequest (["SCRIPT","EXISTS"] ++ map encode (toList script) )
...
sunionstore
    ...
    -> NonEmpty ByteString -- ^ key
    -> m (f Integer)
sunionstore destination key = sendRequest (["SUNIONSTORE"] ++ [encode destination] ++ map encode (toList key) )
...
del
    ...
    => NonEmpty ByteString -- ^ key
    -> m (f Integer)
del key = sendRequest (["DEL"] ++ map encode (toList key) )
...
zrem
    ...
    -> NonEmpty ByteString -- ^ member
    -> m (f Integer)
zrem key member = sendRequest (["ZREM"] ++ [encode key] ++ map encode (toList member) )
...
hmget
    ...
    -> NonEmpty ByteString -- ^ field
    -> m (f [Maybe ByteString])
hmget key field = sendRequest (["HMGET"] ++ [encode key] ++ map encode (toList field) )
...
commandInfo
    :: (RedisCtx m f)
    => NonEmpty ByteString -- ^ commandName
    -> m (f [ByteString])
commandInfo commandName = sendRequest (["COMMAND","INFO"] ++ map encode (toList commandName) )
...
blpop
    ...
    -> Integer -- ^ timeout
    -> m (f (Maybe (ByteString,ByteString)))
blpop key timeout = sendRequest (["BLPOP"] ++ map encode (toList key) ++ [encode timeout] )
