...
--    , exportList (filter (not . blackListed) cmds)
    , exportList cmds
--    , fromString "module Database.Redis.ManualCommands", newline
    , fromString ") where\n\n"
    , imprts, newline
...
fromCmd cmd@Cmd{..}
  ...
  where
    ...
    fun = mconcat
            [ fromString name
            , mconcat (map argumentName cmdArgs)
            , fromString " = "
            , fromString "decode", retType cmd, fromString " <$> "
            , fromString "sendRequest ([\"", fromString cmdName, fromString "\""
...
argumentList (Multiple p@(Pair a a')) = mconcat
    [fromString "] ++ flattenPairs", argumentName p, fromString " ++ ["]
argumentList (Multiple a) =
    mconcat [ fromString "] ++ ", argumentName a, fromString " ++ [" ]
argumentList a@Arg{..} =
    fromString "," `mappend` argumentName a

argumentName :: Arg -> Builder
argumentName a = mappend (fromString " ") $ go a
  where
    go (Multiple a) = go a
